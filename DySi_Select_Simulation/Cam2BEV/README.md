...
- [Use in SIN](#use-in-situation-identification-sin-prototype)
...
├── bev_output                      # *NEW* folder to store intermediate BEV images during the simulation/evaluation with real dataset
├── homography_output               # *NEW* folder to store intermediate homography images when using DeepLab model
...
├── get_ipm.py                      # *NEW* script for obtaining homography images in DeepLab-Mobilenet model
├── predict_bev.py                  # *NEW* script providing interfaces for external use in other software modules
└── trial_ipm.py                    # *NEW* script for testing homography matrices when DeepLab-Mobilenet is used
```
...
## Use in Situation Identification (SIN) Prototype (MT3532)

Python modules `get_ipm.py`, `predict_bev.py` and `trial_ipm.py` are developed to support the use of Cam2BEV in the prototype. 
- `get_ipm.py` provides homography image by converting the parsed front camera semantic segmentation image using IPM (Inverse Perspective Mapping). The output produced from the interface is stored in [homography_output](homography_output). This is only valid if the chosen backbone is DeepLab-Mobilenet.
- `predict_bev.py` provides interfaces for getting the BEV image for the parsed semantic segmentation image (only 2_F configuration as of now) using both DeepLab-Mobilenet and uNetXST model. The weights for the model can be configured (via CLI or YML files) when the corresponding interfaces are called. The inferred BEV images are stored in [bev_output](bev_output) folder for further evaluation.
On CPU-only configuration, the DeepLab-Mobilenet provides the BEV image for further processing with good accuracy and faster inference time (approx. 0.65s-1s). uNetXST architecture is slower when loading since it is a complex model, but performs at similar inference time and better accuracy after the initial loading.
- `trial_ipm.py` is used to play around the homography matrices necessary for DeepLab-Mobilenet model. 
- All created files mentioned above are in the context of thesis MT3532 and are adjusted to be run from the parent project folder "DySi_Select"
- To play around homography conversion and IPM, a `trial_ipm.py` file can be referred.
- To run Cam2BEV training and validation, the steps mentioned by the original authors is still valid.
